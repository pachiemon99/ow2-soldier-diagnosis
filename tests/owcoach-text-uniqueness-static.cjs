...
const csvPath = path.join(root, 'owcoach_diagnostic_text_uniqueness_hints_v50_11.csv');
const contractPath = path.join(root, 'owcoach_text_uniqueness_contract_v50_11.json');
...
for (const row of rows) {
  ...
  if (row.specific_focus.length < 28) throw new Error(`short specific_focus for ${row.target_id}/${row.enemy_id}`);
  if (row.unique_cue.length < 24) throw new Error(`short unique_cue for ${row.target_id}/${row.enemy_id}`);
  if (row.avoid_repeat_note.length < 24) throw new Error(`short avoid_repeat_note for ${row.target_id}/${row.enemy_id}`);
...
if (tokens.size !== 867) throw new Error(`composition_summary_token must be unique per target/enemy, got ${tokens.size}`);
const mustContain = [
  'OWC_TEXT_UNIQUENESS_HINTS_DB',
  'owcApplyDiagnosticTextUniqueness',
  'owcDiagnosticUniquenessHtml',
  'この対面の固有焦点',
  '見分け方：',
  '重複回避：',
  'd=owcApplyDiagnosticTextUniqueness(h,d,target())'
];
...
console.log('Diagnostic text uniqueness static checks passed');
