...
if (lines.length !== 868) throw new Error(`expected 867 data rows, got ${lines.length-1}`);
...
for (const line of lines.slice(1)) {
  ...
  for (const col of required) {
    ...
    if (!v.trim()) throw new Error(`empty ${col} for ${target}/${enemy}`);
  ...
  const rating = Number(cols[idx.matchup_rating]);
...
const mustContain = [
  'OWC_MATCHUP_REASON_DB',
  'owcDetailMatchupReasonHtml',
  'owcCompositionMatchupReasonLines',
  '有利 / 不利理由',
  '相性理由',
  'matchup_reason_contract'
];
