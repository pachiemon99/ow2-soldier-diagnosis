...
const auditRel='data/audits/owcoach_db_quality_score_audit_v50_16.csv';
const summary=path.join(root,'data/audits/owcoach_db_quality_target_summary_v50_16.csv');
const contract=path.join(root,'data/contracts/owcoach_quality_remediation_contract_v50_16.json');
must(fs.existsSync(path.join(root,auditRel)),'missing v50.16 remediation audit CSV');
...
const rows=readExpandedCsv(root,auditRel);
must(rows.length===867,`expected 867 remediation rows, got ${rows.length}`);
['target_id','enemy_id','total_quality_score','quality_band','remediation_applied','quality_version'].forEach(c=>must(Object.prototype.hasOwnProperty.call(rows[0]||{},c),`missing ${c}`));
let cCount=0,dCount=0,eCount=0;
for(const row of rows){ const band=row.quality_band; if(band==='C') cCount++; if(band==='D') dCount++; if(band==='E') eCount++; }
...
must(['50.16.0','50.17.0','50.18.0','50.19.0','50.20.0','50.21.0','50.22.0','50.23.0','50.24.0','50.26.0','50.27.0','50.28.0'].includes(pkg.version),`package version should be 50.16.0, got ${pkg.version}`);
